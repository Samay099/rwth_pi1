...
bool zugGueltig(const int spielfeld[GROESSE_Y][GROESSE_X], const int aktuellerSpieler, const int posX, const int posY)
{
    ...
    for (int j = -1; j <= 1; j++)
    {
        for (int i = -1; i <= 1; i++)
        {
            ...
            if((aufSpielfeld(iCurrentX + j,iCurrentY + i)) && (spielfeld[iCurrentY+ i][iCurrentX + j] == gegner))
            {

                int k = 2;

                while(aufSpielfeld(iCurrentX + k*j, iCurrentY + k*i))
                {
                    if(spielfeld[iCurrentY+ k*i][iCurrentX + k*j] == 0)
                    {
                        break;
                    }
                    else if(spielfeld[iCurrentY+ k*i][iCurrentX + k*j]== aktuellerSpieler)
                    {
                        return true;
                    }
            ...
            if((aufSpielfeld(iCurrentX + j,iCurrentY + i)) && (spielfeld[iCurrentY+ i][iCurrentX + j] == gegner))
            {
                ...
                while(aufSpielfeld(iCurrentX + k*j, iCurrentY + k*i))
                {
                    ...
                    else if(spielfeld[iCurrentY+ k*i][iCurrentX + k*j]== aktuellerSpieler)
                    {
                        for(int n = 0; n < k; n++)
                        {
                            spielfeld[iCurrentY + n*i][iCurrentX + n*j] = aktuellerSpieler;
                        }
                        spielfeld[iCurrentY][iCurrentX] = aktuellerSpieler;
...
void spielen(const int spielerTyp[2])
{
    ...
    {
        bool bErfolgreiche_Runde = false;

        if(spielerTyp[aktuellerSpieler] == MENSCH)
        {
            bErfolgreiche_Runde = menschlicherZug(spielfeld,aktuellerSpieler);
        }
        else
        {
            bErfolgreiche_Runde = computerZug(spielfeld,aktuellerSpieler);
        }

        if( bErfolgreiche_Runde == true)
        {
            zeigeSpielfeld(spielfeld);
        }
        else
    ...
    switch (gewinner(spielfeld))
    {
        ...
        case 2: 
            ...
            break;
        default:
            std::cout<<"das Spiel ist unentschieden"<<std::endl;
            break;
