...
void Medium::ausgabe(std::ostream& stream) const
{
    stream << "ID: " << ID << std::endl;
    stream << "Titel: " << titel << std::endl;

    if (status)
    {
        stream << "Status : Das Medium ist seit dem " << datumAusgeliehen << " an " << personAusgeliehen.getName() << " ausgeliehen." << std::endl;
    }
    else
    {
       stream << "Status: Medium ist zurzeit nicht verliehen." << std::endl;
    }
}
